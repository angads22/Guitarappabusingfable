...
Tests need no audio fixtures: `fretmap/synth.py` synthesizes plucked-string audio with known ground truth (`pluck_midi`, `render_sequence`), and tests assert the pipeline recovers the input. Note that octave doublings inside a chord may be absorbed into the lower note (they share all harmonics), so chord tests assert on **pitch classes**, not exact MIDI sets — see `test_chord_pitch_detection`.
